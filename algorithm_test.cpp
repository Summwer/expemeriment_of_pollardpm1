...
void gen_prime_for_high_index(mpz_t &p,  int p_bit_num, int factor_bit_num, int ind3, ll pt){
    ...
    if(ind3 == 0)
        pre_bit = p_bit_num - floor(log2(pt)) - 3; //bit_p - bit(2 * pt)
    else
        pre_bit = p_bit_num - floor(log2(pt)) - floor(ind3 * log2(3)) - 4; //bit_p - bit(2 * pt * 3^u)
...
//Compare the cost for DSP, som22, pol74 and bis03.
...
void test_expo_growth(){
    ...
    for(int u = 0; u <= ceil(bits/2*log(2)/log(3)) - 31; u+=20){
...
{   
    ...
    //test DSP and som22 with the growth of exponent.
    test_expo_growth();
